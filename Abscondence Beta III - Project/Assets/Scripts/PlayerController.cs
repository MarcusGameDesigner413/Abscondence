...
<<<<<<< HEAD
    ...
    ////-------------------Audio Sources (note lines mentioned in comments might not be 100% accurate due to script updates)
    //when the player takes damage (358ish)
    public AudioSource PlayerTookDamageAudio;

    //when the player dies (265ish)
    public AudioSource PlayerDeathAudio;

    //when the player does a light attack (310ish)
    public AudioSource PlayerLightAttackAudio;
...
<<<<<<< HEAD
        ...
        if (isDying && DeathToMenu)
        {
            if(DeathToMenuIterator <= 1)
            {
            ...
            if(DeathToMenuIterator >= DeathToMenuTimer)
            {
...
<<<<<<< HEAD
    ...
    //idle: default, return to this if not walking
    public void PlayIdleAnimation()
    {
        //Debug.Log("IdleANimation");
        //standard standing still i guess
        meleeSwipe.SetTrigger("IdleAnimation");

    }

    //execution: plays when the player executes the trooper, should automatically leave
    public void PlayExecutionAnimation()
    {
        Debug.Log("haha im babk baby");
        //stab stab the trooper when they on the ground
        meleeSwipe.SetTrigger("ExecutionAnimation");

    }

    //death: plays when the player looses all health
    public void PlayDeathAnimation()
    {
        Debug.Log("U ARE DED");
        //aaaaaaa im dead
        meleeSwipe.SetTrigger("DeathAnimation");

    }

    //walk cycle: plays when walking
    public void PlayWalkCycleAnimation()
    {
        //Debug.Log("WALK ANIMATION");
        //we be making our way down town... walkin
        meleeSwipe.SetTrigger("WalkCycleAnimation");

    }

    //interact: plays when interactable happens EVERYTHING EXCEPT EXECUTION YOU PRESS E FOR
    public void PlayInteractAnimation()
    {
        Debug.Log("im doing things");
        //im doing things
        meleeSwipe.SetTrigger("InteractAnimation");
